...
dbapi.saveInsAccount = function (account, config, serial = '') {
  ...
  !follow && (follow = {
    ...
    optBeforeInfo: { // 操作后关注设置信息
      optBeforeStatus: 0, //关注后操作总开关
      beforeViewStatus: false, // 关注后操作看故事开关 0=关 1=开
      beforeViewMin: 1, // 看故事随机数最小值
      beforeViewMax: 1, // 看故事随机数最大值
      beforeThumbStatus: false, // 关注后操作点赞开关 0=关 1=开
      beforeThumbMin: 1, // 点赞随机数最小值
      beforeThumbMax: 1, // 点赞随机数最大值
      beforeThumbTotal: 1, // 点赞操作前x条帖子
      beforeThumbChoice: 1, // 点赞操作选择前y条帖子
      beforeCommentStatus: false, // 关注后操作评论开关 0=关 1=开
      beforeCommentMin: 1, // 评论随机数最小值
      beforeCommentMax: 1, // 评论事随机数最大值
      beforeCommentTotal: 1, // 评论操作前x条帖子
      beforeCommentChoice: 1, // 评论操作随机选择y条帖子操作
      beforeCommentTxt: '', // 评论操作的文字内容(旋转语法)
    },
    optAfterInfo: { // 操作后关注设置信息
      optAfterStatus: 0, //关注后操作总开关
      ...
      afterFollowMinNum: 1, // 添加关注后推荐用户数量
      afterFollowMaxNum: 4 // 添加关注后推荐用户数量
    }
  })
...
dbapi.clearInsAccount = function () {
  return db.run(r.table('insAccount').delete())
}
...
dbapi.clearLogsByAccount = function (account, type) {
  return db.run(r.table('insLogs')
    .orderBy({index: r.desc('created')})
    .filter({
      account,
...
dbapi.saveOrUpdateInsaccountRecord = function (account, type, date, record) {
  console.log(account, type, date, record)
  console.log('===========================================================================')
