...
dbapi.saveInsAccount = function(account, config, serial = '') {
  ...
  !unfollow && (unfollow = {
    ...
    minPage: 0,        // 每页取关最小人数
    maxPage: 7,        // 每页取关最大人数
    minEnter: 0,        // 进入详情页操作最小范围
    maxEnter: 0,        // 进入详情页操作最大范围
    excute: {
      start: '00:00',
      end: '23:59'
    },        // 执行时间
...
dbapi.getAllInsAccount = function() {
  return db.run(r.table('insAccount').orderBy(r.desc('created')))
}
...
dbapi.getAllLogs = function(page, limit) {
  return db.run(r.table('insLogs').orderBy(r.desc('created'))
    .eqJoin(
      'serial',
      r.table('devices')
    ))
...
dbapi.getAllLogsNoJoin = function() {
  return db.run(r.table('insLogs'))
}

dbapi.getLogsBySerial = function(serial, type) {
  return db.run(r.table('insLogs').orderBy(r.desc('created')).filter({serial, type}).eqJoin(
    'serial',
    r.table('devices')
  ))
}
...
dbapi.getLogsByAccount = function(account, type) {
  ...
  return db.run(r.table('insLogs').orderBy(r.desc('created')).filter(filter).eqJoin(
    'serial',
    r.table('devices')
  ))
}

dbapi.getLogsByAccountData = function(account) {
  return db.run(r.table('insLogs').orderBy(r.desc('created')).filter({account}))
}

dbapi.getLogsByAccountLimit = function(account, type, page, limit) {
  return db.run(r.table('insLogs').orderBy(r.desc('created')).filter({
    account,
    type
  }).skip((page - 1) * limit).limit(limit).eqJoin(
    'serial',
    r.table('devices')
  ))
}
