...
dbapi.saveDeviceIns = function(serial, insId) {
  return db.run(r.table('deviceIns').insert({
    serial,
    insId
  }))
}

dbapi.getDeviceInsBySerial = function(serial) {
  return db.run(r.table('deviceIns').get(serial))
}

dbapi.updateDeviceIns = function(serial, insId) {
  return db.run(r.table('deviceIns').get(serial).update({
    insId
  }))
}
...
dbapi.getDeviceSerialList = function() {
