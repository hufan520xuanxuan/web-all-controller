  ...
  accountlist.map(account => {
    ...
    if (followStatus && followExcute.start && followExcute.end) {
      ...
      job.follow = createInsJob(start, end, followWeekday, (time) => {
        log.info('start', account, time)
      }, (time) => {
        log.info('end', account, time)
      })
    }
...
function updateInsJob(account = {}) {
  ...
  if(jobs[account.account]) {
    jobs[account.account].follow.start.cancel()
    jobs[account.account].follow.end.cancel()
    jobs[account.account].unfollow.start.cancel()
    jobs[account.account].unfollow.end.cancel()
  }
