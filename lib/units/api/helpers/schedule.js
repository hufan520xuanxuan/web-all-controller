...
function initInsShedule(accountlist = []) {
  accountlist.map(account => {
    ...
    if (postStatus && serial) {
      postList.map(item => {
        // 判断帖子是否有效
        if (Number(moment(item.postTime)) > Date.now() && item.res && item.imgList.length) {
          job.post.postList.push(createPostJob(item.postTime, () => {
            insScript.startPost(account, serial, item)
          }))
        }
      })
      if (randomPost.postTime && Number(moment(randomPost.postTime)) > Date.now()
        && randomPost.res && randomPost.imgList.length) {
...
function updateInsJob(account = {}) {
  ...
  if (postStatus && serial) {
    ...
    if (randomPost.postTime && Number(moment(randomPost.postTime)) > Date.now() && randomPost.res && randomPost.imgList.length) {
      job.post.randomPost = createPostJob(randomPost.postTime, () => {
        insScript.startPost(account.account, serial, randomPost)
      })
    }
