...
function startFollow(account, serial, followConfig, isException = false) {
  ...
  let randomDay = golbalData.get(serial, 'randomDay') ||
    Math.floor(Math.random() * (config.maxDay - config.minDay + 1)) + config.minDay

  let record = golbalData.get(serial, 'record')
  if (record !== '' && randomDay >= record) return
  golbalData.set(serial, {randomDay})
