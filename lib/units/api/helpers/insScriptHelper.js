...
function checkStartScript(account, serial, config, type = 1, isException = false) {
  ...
  console.log(validTime, serial, status)
...
function formatFollowShell(params = {}) {
  ...
  data.testAdd.status = data.testAdd.status ? 1 : 0
  data.swipeInfo.status = data.swipeInfo.status ? 1 : 0
  data.optBeforeInfo.optBeforeStatus = data.optBeforeInfo.optBeforeStatus ? 1 : 0
  data.optAfterInfo.optAfterStatus = data.optAfterInfo.optAfterStatus ? 1 : 0
...
function formatUnfollowShell(params = {}) {
  ...
  const list = []
  if (reslist.list) {
    reslist.list.forEach(res => {
      list.push(res.title)
    })
  }
  reslist.list = list
  reslist.status = reslist.status ? 1 : 0
