...
function getUsers(req, res) {
  dbapi.getUsersNotAdmin().then(cursor => {
...
function updateUserDevice(req, res) {
  let {
    serial,
    email
  } = req.body
  dbapi.getUserBySerial(serial).then(cursor => {
    return Promise.promisify(cursor.toArray, cursor)()
      .then(function(list) {
        if (list.length) {
          let user = list[0]
          let devices = user.devices
          let index = devices.indexOf(serial)
          devices.splice(index, 1)
          dbapi.updateUserDeviceByEmail(user.email, devices)
        }
      })
  })

  if(email) {
    dbapi.getUserByEmail(email).then(user => {
      let devices = user.devices
      devices.push(serial)
      dbapi.updateUserDeviceByEmail(email, devices)
      res.json({
        success: true
      })
    })
...
function logout(req, res) {
  res.session = null
  res.clearCookie(process.env.SSID || 'ssid')
  return res.json({
    success: true
  })
}
