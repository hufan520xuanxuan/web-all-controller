    ...
    if (ret) {
      ...
      if (funcNameList[type - 1]) {
        ...
        if (funcNameList[type - 1] !== 'unfollow') {
          resource = config[funcNameList[type - 1]].insUsers['resource' + resourceType].res.find(item => item.res === resName && item.type === resType)
        } else {
          resource = config.unfollow.reslist.list.find(item => item.title === resName)
        }
