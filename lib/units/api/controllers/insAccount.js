      ...
      .then((list) => {
        ...
        Promise.all(chain).then(() => {
          ...
          return res.json({
            ...
            total: Math.ceil(newAccountList.length / limit)
          })
        })
      })
  })
  ...
  return dbapi.getInsAccountRecord(item.account).then(cursor => {
    return Promise.promisify(cursor.toArray, cursor)()
      .then(records => {
        ...
        let todayFollowRecord = _.find(followRecords, item => item.created === today)

        let unfollowRecords = records.filter(item => item.type === 2)
        let todayUnfollowRecord = _.find(unfollowRecords, item => item.created === today)

        let thumbRecords = records.filter(item => item.type === 3)
        let todayThumbRecord = _.find(thumbRecords, item => item.created === today)

        let commentRecords = records.filter(item => item.type === 4)
        let todayCommentRecord = _.find(commentRecords, item => item.created === today)

        let messageRecords = records.filter(item => item.type === 5)
        let todayMessageRecord = _.find(messageRecords, item => item.created === today)
        let postRecords = records.filter(item => item.type === 6)
        let todayPostRecord = _.find(postRecords, item => item.created === today)
        let browseRecords = records.filter(item => item.type === 7)
        let todayBrowseRecord = _.find(browseRecords, item => item.created === today)
...
function getInsAccountByAccount(req, res) {
  ...
  getInsAccountByAccountService(account).then(ret => {
    if (ret) {
      ...
      delete ret.config.unfollow.reslist
      return res.json({
        success: true,
        data: ret
      })
...
function updateInsAccountConfigByAccount(req, res) {
  ...
  dbapi.getInsAccountByAccount(account).then(ret => {
    ...
    config.unfollow.reslist = ret.config.unfollow.reslist
    dbapi.updateInsAccountConfigByAccount(account, config).then(updateRet => {
      if (updateRet) {
        if (ret && ret.serial) {
          ret.config = config
...
function clearInsAccount(req, res) {
  dbapi.clearInsAccount().then(ret => {
    if (ret) {
      return res.json({
        success: true
      })
 ...
 * @param res
 */
function clearLogsByAccount(req, res) {
  let {account, type} = req.body

  dbapi.clearLogsByAccount(account, type).then(() => {
    res.json({
      success: true
    })
  }).catch(function (err) {
...
function updatePostResource(serial, message) {
  ...
  dbapi.getInsAccountByAccount(account).then(ret => {
    ...
    console.log('zzz=' + Number(postType))
...
function getResourceBlackList(req, res) {
  ...
  dbapi.getInsAccountByAccount(account).then(ret => {
    if (ret) {
      ...
      resList.sort((a, b) => b.created - a.created)
      return res.json({
        success: true,
        data: resList.slice((page - 1) * limit, page * limit),
        allNum: resList.length
...
function clearResource(req, res) {
  let {
    type,
    resourceType,
    account
  ...
  dbapi.getInsAccountByAccount(account).then(ret => {
    if (ret) {
      let config = ret.config
      config[funcNameList[type - 1]].insUsers['resource' + resourceType].res = []
      dbapi.updateInsAccountConfigByAccount(account, config).then(ret => {
        return res.json({
          success: true,
          msg: '操作成功'
  ...
  dbapi.getInsAccountByAccount(account).then(ret => {
    if (ret) {
      ...
      if (funcNameList[type - 1]) {
        let config = ret.config
        let resource
        if (funcNameList[type - 1] !== 'unfollow') {
          resource = _.find(config[funcNameList[type - 1]].insUsers['resource' + resourceType].res, item => item.res === resName && item.type === resType)
        } else {
          resource = _.find(config.unfollow.reslist.list, item => item.title === resName)
        }
...
//加入黑名单
async function putBlackList(serial, message) {
  //解析message
  let {
    resName,
    resourceType,
  ...
  //获取对应账户的数据
  const ret = await dbapi.getInsAccountByAccount(account)
  if (ret && funcNameList[type - 1]) {
    let config = ret.config
    ...
    if (blackList.findIndex(i => i.blackName === blackName && i.res === resName && i.type === resType) < 0) {
      ...
      // config[funcNameList[type - 1]].insUsers['resource' + resourceType] = resource
      // log.error('mh=存数据 私密集合=', resource.privateBlackList)
      // lo
      // g.error('mh=存数据 资源集合=', resource.blackList)
      try {
        await dbapi.updateInsAccountConfigByAccount(account, config)
      } catch (e) {
        log.error('mh=存数据 报错信息=' + e)
      }
    }
  }
}
...
function getUnfollowWhiteList(req, res) {
  let {account = '', search = '', page = 1, limit = 5} = req.body
  ...
  getInsAccountByAccountService(account).then(ret => {
    if (ret) {
      ...
      return res.json({
        ...
        total: Math.ceil(whitelist.length / limit),
        allNum: whitelist.length
      })
    } else {
      return res.status(404).json({
        success: false
...
function clearUnfollowWhiteList(req, res) {
  let {
    account
  } = req.body
  ...
  dbapi.getInsAccountByAccount(account).then(ret => {
    if (ret) {
      ...
      config.unfollow.whitelist.list = ''
      dbapi.updateInsAccountConfigByAccount(account, config).then(ret => {
        return res.json({
          success: true,
          msg: '操作成功'
...
function getUnfollowResList(req, res) {
  let {account = '', search = '', page = 1, limit = 5} = req.body

  getInsAccountByAccountService(account).then(ret => {
    if (ret) {
      let reslist = ret.config.unfollow.reslist.list || []
      if (search) {
        reslist = _.filter(reslist, (item) => item.title.includes(search))
      }
      return res.json({
        success: true,
        data: reslist.slice((page - 1) * limit, page * limit),
        total: Math.ceil(reslist.length / limit),
        allNum: reslist.length
      })
    } else {
      return res.status(404).json({
    ...
    .catch(function (err) {
      log.error('getUnfollowResList操作失败', err.stack)
      res.status(500).json({
        success: false
      })
    })
...
function updateUnfollowResList(req, res) {
  let {account = '', list = []} = req.body
  ...
  getInsAccountByAccountService(account).then(ret => {
    if (ret) {
      ...
      let oldWhitelist = config.unfollow.reslist.list || []
      let reslist = []
      list.map(item => {
        let index = oldWhitelist.findIndex(oldItem => {
          return oldItem.title === item.title || oldItem.title === item
        })
        if (index >= 0) {
          if (!_.isString(item)) oldWhitelist[index] = {...item}
        } else {
          reslist.push({
            title: item,
      ...
      reslist.push(...oldWhitelist)
      config.unfollow.reslist.list = reslist
      dbapi.updateInsAccountConfigByAccount(account, config).then(updateRet => {
        if (updateRet) {
          if (ret && ret.serial) {
            ret.config = config
...
function getUnfollowResListStatus(req, res) {
  let {account = ''} = req.body

  getInsAccountByAccountService(account).then(ret => {
    if (ret) {
      let status = ret.config.unfollow.reslist.status
      return res.json({
        success: true,
        data: status
      })
...
function updateUnfollowResListStatus(req, res) {
  let {account = '', status = 0} = req.body
  ...
  getInsAccountByAccountService(account).then(ret => {
    if (ret) {
      ...
      config.unfollow.reslist.status = status
      dbapi.updateInsAccountConfigByAccount(account, config).then(updateRet => {
        if (updateRet) {
          if (ret && ret.serial) {
            ret.config = config
        ...
        .catch(function (err) {
          log.error('updateUnfollowResListStatus操作失败', err.stack)
          res.status(500).json({
            success: false
          })
        })
...
function delUnfollowResList(req, res) {
  let {
    resName,
    account
  } = req.body
  ...
  dbapi.getInsAccountByAccount(account).then(ret => {
    if (ret) {
      let config = ret.config
      let list = config.unfollow.reslist.list || []
      let index = list.findIndex(item => _.isEqual(item, resName))
      list.splice(index, 1)
      config.unfollow.reslist.list = list
      dbapi.updateInsAccountConfigByAccount(account, config).then(ret => {
        return res.json({
          success: true,
          msg: '操作成功'
...
function clearUnfollowResList(req, res) {
  let {
    account
  } = req.body
...
function exportBlackList(req, res) {
  console.log(req)
  let {
    account,
    resourceType,
    ...
    if (ret) {
      ...
      resList.forEach(item => {
        rows.push([
          item.blackName,
          item.res,
          item.status === 9 ? '私密' : getResType(item.type)
        ])
      })
      conf.rows = rows;
...
module.exports = {
  ...
  getUnfollowResList,
  updateUnfollowResList,
  getUnfollowResListStatus,
  updateUnfollowResListStatus,
  delUnfollowResList,
  ...
  clearInsAccount,
  clearLogsByAccount,
  clearUnfollowWhiteList,
  clearUnfollowResList,
  exportBlackList
}
