...
function updateReourceStatus(req, res) {
  ...
  dbapi.getInsAccountByAccount(account).then(ret => {
    if (ret) {
      ...
      insUsers.resource1.blackSecret = resource1.blackSecret || 0
      insUsers.resource1.blackFollow = resource1.blackFollow || 0

      insUsers.resource2.level = resource2.level
      insUsers.resource2.status = resource2.status
      insUsers.resource2.blackSecret = resource2.blackSecret || 0
      insUsers.resource2.blackFollow = resource2.blackFollow || 0

      insUsers.resource3.level = resource3.level
      insUsers.resource3.status = resource3.status
      insUsers.resource3.blackSecret = resource3.blackSecret || 0
      insUsers.resource3.blackFollow = resource3.blackFollow || 0
...
function getInsAccountBySerial(req, res) {
  ...
  if (serial) {
    dbapi.getInsAccountBySerial(serial).then(cursor => {
      return Promise.promisify(cursor.toArray, cursor)()
        .then(list => {
          let account = list.length ? list[0] : null
          let config = {
