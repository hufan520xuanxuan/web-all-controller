...
function updateInsAccountMinDayAndMaxDay(account, config) {
  ...
  if (followAddDay && followTopDay && config.follow.status) {
    let random = Math.floor(Math.random() * (followTopDay - followAddDay + 1)) + followAddDay
    config.follow.minDay += random
    config.follow.maxDay += random
  }

  // 取关
  let unfollowAddDay = Number(config.unfollow.addDay)
  let unfollowTopDay = Number(config.unfollow.topDay)
  if (unfollowAddDay && unfollowTopDay && config.unfollow.status) {
    let random = Math.floor(Math.random() * (unfollowAddDay - unfollowTopDay + 1)) + unfollowTopDay
    config.unfollow.minPerson += random
    config.unfollow.maxPerson += random
  }

  // 点赞
  let thumbAddDay = Number(config.thumb.addDay)
  let thumbTopDay = Number(config.thumb.topDay)
  if (thumbAddDay && thumbTopDay && config.thumb.status) {
    let random = Math.floor(Math.random() * (thumbTopDay - thumbAddDay + 1)) + thumbAddDay
    config.thumb.minDay += random
    config.thumb.maxDay += random
  }

  // 评论
  let commentAddDay = Number(config.comment.addDay)
  let commentTopDay = Number(config.comment.topDay)
  if (commentAddDay && commentTopDay && config.comment.status) {
    let random = Math.floor(Math.random() * (commentTopDay - commentAddDay + 1)) + commentAddDay
    config.comment.minDay += random
    config.comment.maxDay += random
  }

  // 私信
  let messageAddDay = Number(config.message.addDay)
  let messageTopDay = Number(config.message.topDay)
  if (messageAddDay && messageTopDay && config.message.status) {
    let random = Math.floor(Math.random() * (messageTopDay - messageAddDay + 1)) + messageAddDay
    config.message.minDay += random
    config.message.maxDay += random
  }
