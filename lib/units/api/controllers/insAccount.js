        ...
        .then(function(list) {
          ...
          list.map(device => {
            devices.push({
              ...
              model: device.model,
              notes: device.notes,
              isOwner: req.user.devices.includes(device.serial)
            })
          })
          res.json({
            success: true
...
function getAllLogs(req, res) {
  ...
  console.time('getAllLogs')
  dbapi.getAllLogs(page, limit)
    .then(function(cursor) {
      console.timeEnd('getAllLogs')
      return Promise.promisify(cursor.toArray, cursor)()
        .then(function(list) {
...
function updateInsAccountMinDayAndMaxDay(account, config) {
  ...
  if (followAddDay && config.follow.maxDay < followTopDay && config.follow.status) {
  ...
  // if (unfollowAddDay && config.unfollow.maxPerson < unfollowTopDay && config.unfollow.status) {
  //   config.unfollow.minPerson += unfollowAddDay
  //   config.unfollow.maxPerson += unfollowAddDay
  // }
  // if (config.unfollow.minPerson > unfollowTopDay) config.unfollow.minPerson = unfollowTopDay
  // if (config.unfollow.maxPerson > unfollowTopDay) config.unfollow.maxPerson = unfollowTopDay

  // 点赞
  let thumbAddDay = Number(config.thumb.addDay)
  let thumbTopDay = Number(config.thumb.topDay)
  if (thumbAddDay && config.thumb.minDay < thumbTopDay && config.thumb.status) {
  ...
  if (commentAddDay && config.comment.maxDay < commentTopDay && config.comment.status) {
  ...
  if (messageAddDay && config.message.maxDay < messageTopDay && config.message.status) {
...
function closeResource(serial, message) {
  ...
  dbapi.getInsAccountByAccount(account).then(ret => {
    if (ret) {
      ...
      log.info(`操作${typeName}的资源`)
      if (typeName) {
        let config = ret.config
        let resource = config[typeName].insUsers['resource' + resourceType].res.find(item => item.res === resName)
        resource.status = 0
        log.info(resource)
        dbapi.updateInsAccountConfigByAccount(account, config)
      }
...
function putBlackList(serial, message) {
  ...
  dbapi.getInsAccountByAccount(account).then(ret => {
    if (ret) {
      ...
      if (typeName) {
        let config = ret.config
        let resource = config[typeName].insUsers['resource' + resourceType]
        let blackList = resource.blackList || []

        // 该用户不存在黑名单中
        if (!blackList.includes(blackName)) {
          blackList.push({
            blackName,
            res: resName,
            type: resType
