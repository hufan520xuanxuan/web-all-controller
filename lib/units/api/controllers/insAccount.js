...
function getUnfollowResList(req, res) {
  ...
  getInsAccountByAccountService(account).then(ret => {
    if (ret) {
      let reslist = ret.config.unfollow.reslist.list ? ret.config.unfollow.reslist.list.split(',') : []
      if (search) {
...
function updateUnfollowResList(req, res) {
  ...
  getInsAccountByAccountService(account).then(ret => {
    if (ret) {
      ...
      let oldWhitelist = config.unfollow.reslist.list ? config.unfollow.reslist.list.split(',') : []
      let reslist = []
      list.map(item => {
        if (!oldWhitelist.includes(item)) {
          reslist.push(item)
      ...
      config.unfollow.reslist.list = reslist.join(',')
      dbapi.updateInsAccountConfigByAccount(account, config).then(updateRet => {
        if (updateRet) {
          if (ret && ret.serial) {
            ret.config = config
...
function delUnfollowResList(req, res) {
  ...
  dbapi.getInsAccountByAccount(account).then(ret => {
    if (ret) {
      let config = ret.config
      let list = config.unfollow.reslist.list ? config.unfollow.reslist.list.split(',') : []
      let index = list.findIndex(item => item === resName)
      list.splice(index, 1)
      config.unfollow.reslist.list = list.join(',')
      dbapi.updateInsAccountConfigByAccount(account, config).then(ret => {
        return res.json({
          success: true,
          msg: '操作成功'
