  ...
  return dbapi.getLogsByAccountData(item.account).then(cursor => {
    return Promise.promisify(cursor.toArray, cursor)()
      .then(logs => {
        ...
        // item.follow = { status: item.follow.status }
        // item.unfollow = { status: item.unfollow.status }
        // item.comment = { status: item.comment.status }
        // item.message = { status: item.message.status }
        // item.post = { status: item.post.status }
        // item.thumb = { status: item.thumb.status }
    ...
    .then(function(cursor) {
      return Promise.promisify(cursor.toArray, cursor)()
        .then(function(list) {
          ...
          let newList = list.splice((page - 1) * limit, limit)
          let logs = []
          newList.map(item => {
