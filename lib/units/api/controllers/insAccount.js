    ...
    if (ret) {
      ...
      dbapi.updateInsAccountConfigByAccount(account, config).then(updateRet => {
        if (updateRet) {
          ...
          var validTime = moment().isBetween(today + config.follow.excute.start, today + config.follow.excute.end)
          if (validTime && ret.serial) {
            log.info('有效时间范围内，立即执行脚本')
            insScript.startFollow(account, ret.serial, config.follow)
          }
          else {
            log.info('不在有效时间范围内或未指定设备，不执行脚本')
          }
