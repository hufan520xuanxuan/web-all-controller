        ...
        dbapi.getAllLogsNoJoin().then(cursor => {
          return Promise.promisify(cursor.toArray, cursor)()
            .then((logs) => {
              let newLogs = []
              list.map((item) => {
                newLogs.push(getAccountDataService2(item, logs))
              })
              console.log(newLogs)
              return res.json({
                success: true,
...
function getAccountDataService2(item, logs) {
  let newLogs = logs.filter(item => item.account === item.account && item.status === 1)
  ...
  return dbapi.getLogsByAccountData(item.account).then(cursor => {
    return Promise.promisify(cursor.toArray, cursor)()
      .then(logs => {
        ...
        item.config.post = {status: item.config.post.status}
        item.config.thumb = {status: item.config.thumb.status}
        item.config.browse = {status: item.config.browse.status}
