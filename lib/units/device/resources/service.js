      ...
      return getPath()
        ...
        .catch(function () {
          ...
          return promiseutil.periodicNotify(
            ...
            // 获取分身软件的安装情况
            .then(function () {
              return adb.shell(options.serial, 'dumpsys package com.bly.dkplat | ' +
                'grep versionName')
            }).timeout(10000)
            .then(function (out) {
              streamutil.readAll(out)
                .then(function (buffer) {
                  log.error('分身信息=' + buffer.toString())
                  var version = buffer.toString().replace('versionName=', '').trim()
                  log.error('分身检测结果=' + version + '对比一下结果=' + (version === '终结版'))
                  if (version !== '终结版') {
                    log.error('开始安装分身')
                    adb.uninstall(options.serial, resource.dkPkg)
                      ...
                      .then(function () {
                        promiseutil.periodicNotify(
                          adb.install(options.serial, resource.dkApk), 20000
                        )
                      })
                  }
                })
