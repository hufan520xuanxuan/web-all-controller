  ...
  .define(function (options, adb, router, push) {
    ...
    router.on(wire.InstallMessage, function (channel, message) {
      ...
      function pushApp() {
        ...
        // 这里需要动态地捕捉内容长度，否则就会有风险失去了一些最初的大块
    ...
    router.on(wire.PushPicMessage, function (channel, message) {
      ...
      var source = new stream.Readable().wrap(req)
      var target = '/storage/emulated/0/DCIM/' // 默认导入到手机上相册的目录下
      let imgs = message.href.split('/')
      let img = imgs[imgs.length - 1]
      log.error('source:' + source + '=req=' + req)
      // adb push文件真正实现逻辑的地方

      adb.push(options.serial, source, target + img)
        .then(function () {
          push.send([
            channel
            , reply.okay('success')
    ...
    // 导入视频或文件
    router.on(wire.PushFileMessage, function (channel, message) {
      var target = '/storage/emulated/0/DCIM/' // 默认导入到手机上相册的目录下
      adb.push(options.serial, message.path, target + message.name)
        .then(function () {
          console.log('文件传递完成')
        })
    })
