  ...
  .define(function(options, adb, router, push) {
    ...
    router.on(wire.InstallMessage, function(channel, message) {
      ...
      function pushApp() {
        ...
        console.log('url', url.resolve(options.storageUrl, message.href))
    ...
    router.on(wire.PushPicMessage, function(channel, message) {
      console.log(channel, message.href)
      // log.info('push message "%s"', message.packageName)
      //
      var reply = wireutil.reply(options.serial)
      console.log(url.resolve(options.storageUrl, message.href))
      ...
      var source = new stream.Readable().wrap(req)
      console.log('soso', source)

      let imgs = message.href.split('/')

      let img = imgs[imgs.length - 1]
      console.log(img)
      adb.push(options.serial, source, '/storage/emulated/0/DCIM/' + img)
        .then(function() {
          push.send([
            channel
            , reply.okay('success')
