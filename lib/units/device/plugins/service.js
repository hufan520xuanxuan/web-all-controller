...
module.exports = syrup.serial()
  ...
  .define(function(options, adb, router, push, apk) {
    ...
    function handleEnvelope(data) {
      var envelope1 = apk.mao.Envelope.decode(data)
      log.error('shoudaoleshuju=' + envelope1.id + '=' + envelope1.type)
      if (envelope1.type === 99) {
        var message1 = apk.wire.GetClipboardResponse.decode(envelope1.message)
        log.error('读取数据成功')
        log.error('读取到text=' + message1.text)
        // push.send([
        //   wireutil.global
    ...
    function openIns() {
      ...
      return callService(util.format(
        ...
        .then(function(conn) {
          ...
          ins.reader.on('data', data => {
            var envelope0 = apk.mao.Envelope.decode(data)
            log.error('shoudaoshuju=' + envelope0.id + '=' + envelope.type)
          ...
          conn.onopen = function(event) {
            // 发送一个初始化消息
            conn.send('I am the client and I\'m listening!')
            conn.write('woshi')
          }

          // log.error('socket=' + conn.sock)
          // conn.write.on('我是谁啊', 'utf-8')
          // ins.writer = new ms.DelimitingStream()
          // ins.writer.pipe(conn)
          //
          // ins.writer.write('woshi')
          // log.error('发送完毕。。')
          ins.writer = new ms.DelimitingStream()
          ins.writer.pipe(conn)
          console.log('-----------------》')
          console.log(Object.keys(ins.writer))
<<<<<<< HEAD
          console.log(ins.writer.write)
          ins.writer.write('朋友你好')
=======
          console.log(ins.writer)
>>>>>>> 67806781de4f9764bc0f9bed4d85c4b2f46169f6
          return prepareForServiceDeath(conn)
        })
    }
