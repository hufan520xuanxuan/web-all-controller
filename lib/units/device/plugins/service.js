  ...
  .define(function (options, adb, router, push, sdk, apk) {
      ...
      return adb.shell(options.serial, util.format(
        ...
        .then(function (out) {
          return streamutil.findLine(out, /^Error/)
            ...
            .timeout(10000)
            .then(function (line) {
              log.info('第一次启动报错=' + line)
              log.info('第一次启动报错判断=' + line.indexOf('--user'))
              if (line.indexOf('--user') !== -1) {
                log.info('第二次启动服务')
                log.info('第二次启动服务=' + intent)
                log.info('第二次启动服务指令=' + util.format('am startservice %s', intent))
                return adb.shell(options.serial, util.format(
                  'am %s %s'
                  , startServiceCmd, intent
                ))
                ...
                log.info('第三次启动服务')
                log.info('第三次启动服务=' + intent)
                log.info('第三次启动服务指令=' + util.format('am startservice %s', intent))
                adb.shell(options.serial, 'am start -n jp.co.cyberagent.stf/.IdentityActivity')
                  .timeout(5000)
                  .then(function () {
                    log.info('然后开启服务')
                    return adb.shell(options.serial, 'am startservice  -n jp.co.cyberagent.stf/.Service')
                  })
              }
            })
            .catch(streamutil.NoSuchLineError, function () {
    ...
    function checkInsMessage(serial, message) {
      ...
      if (status === 5) {
        ...
        // 黑名单
          async () => await insAccountController.putBlackList(serial, message)
      } else if (type === 6 && status === 2) {
        log.error('zz=' + message)
        // 发帖成功
        insAccountController.updatePostResource(serial, message)
      } else if (status === 2) {
        insAccountController.setInsAccountRecord(serial, message)
      }
      // else if (status === 2) {
      //   // 脚本执行成功
      //   if (type < 8) {
