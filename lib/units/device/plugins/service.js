      ...
      if (envelope.id !== null) {
        ...
        switch (envelope.type) {
          case apk.wire.MessageType.EVENT_SHELL_REPORT:
            message = apk.wire.ShellReportEvent.decode(envelope.message)
            insAccountController.saveLog({
              ...message,
              serial: options.serial
            })
            log.error('服务端收到客户端数据=' + options.serial + message.type + '=' + message.serial + '=' + message.msg + '='
      ...
      if (status === 0) {
        ...
        insAccountController.getInsAccountByAccountService(account).then(insAccount => {
          switch(+type) {
            ...
            case 2:
              // 取关
              let validTime = moment().isBetween(today + insAccount.config.unfollow.excute.start, today + insAccount.config.unfollow.excute.end)
              if (validTime) {
                log.info('有效时间范围内，重新执行脚本')
