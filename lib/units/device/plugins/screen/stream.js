...
module.exports = syrup.serial()
  ...
  .define(function (options, adb, minicap, display, screenOptions) {
    ...
    FrameProducer.prototype.updateProjection = function (width, height, rate = 0.8) {
      ...
      this.frameConfig.virtualWidth = Math.round(width * rate)
      this.frameConfig.virtualHeight = Math.round(height * rate)
      this._configChanged()
    }
            ...
            return new Promise(function (resolve, reject) {
              switch (ws.readyState) {
                ...
                case WebSocket.OPEN:
                  ...
                  // 在websocket的ws会话发送图片二进制数据到web端时丢弃1/2的数据
                  index++
