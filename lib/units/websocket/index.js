  ...
  io.on('connection', function (socket) {
    ...
    var messageListener = wirerouter()
      .on(wire.DeviceLogMessage, function (channel, message) {
        if (devices.includes(message.serial) || user.admin) {
    ...
    new Promise(function (resolve) {
      socket.on('disconnect', resolve)
        ...
        .on('device.note', function (data) {
          return dbapi.setDeviceNote(data.serial, data.note)
            ...
            .then(function (device) {
              if (device) {
                if (devices.includes(device.serial) || user.admin) {
        ...
        // 导入文件
        .on('device.push', function (channel, responseChannel, data) {
          joinChannel(responseChannel)
          push.send([
            channel
            , wireutil.transaction(
              responseChannel
              , new wire.PushPicMessage(data)
            )
          ])
        })
        // 导入视频或文件
        .on('device.file', function (channel, responseChannel, data) {
          joinChannel(responseChannel)
          push.send([
            channel
            , wireutil.transaction(
              responseChannel
              , new wire.PushFileMessage(data)
            )
          ])
        })
