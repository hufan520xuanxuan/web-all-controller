    ...
    , link: function(scope, element) {
      ...
      scope.addResource = function(type) {
        ...
        if (scope[usersType]) {
          ...
          if (resourceType) {
            console.log(scope.insAccount.config.comment.insUsers, resourceType)
            users.map(user => {
      ...
      scope.addlevel = function(type, index) {
        ...
        let level = scope.insAccount.config.comment.insUsers[resourceType].res[index].level
        if (level < 10) {
          ++scope.insAccount.config.comment.insUsers[resourceType].res[index].level
          // $http.post('/app/api/v1/ins/update_config', scope.insAccount)
        }
      }
      ...
      scope.delInsUser = function(type, index) {
        ...
        if (ret) {
          scope.insAccount.config.comment.insUsers[resourceType].res.splice(index, 1)
          // $http.post('/app/api/v1/ins/update_config', scope.insAccount)
        }
      }
      ...
      scope.addResLevel = (type) => {
        ...
        let level = scope.insAccount.config.comment.insUsers[resourceType].level
        if (level < 10) {
          ++scope.insAccount.config.comment.insUsers[resourceType].level
          // $http.post('/app/api/v1/ins/update_config', scope.insAccount)
        }
      }
      ...
      scope.save = function() {
        let insUsers = scope.insAccount.config.comment.insUsers
