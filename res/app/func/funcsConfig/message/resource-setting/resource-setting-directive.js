  ...
  return {
    ...
    , link: function(scope, element) {
      ...
      scope.addlevel = function(type, index) {
        ...
        let level = scope.insAccount.config.message.insUsers[resourceType].res[index].level
        if (level < 10) {
          ++scope.insAccount.config.message.insUsers[resourceType].res[index].level
          // $http.post('/app/api/v1/ins/update_config', scope.insAccount)
        }
      }
      ...
      scope.delInsUser = function(type, index) {
        ...
        if (ret) {
          scope.insAccount.config.message.insUsers[resourceType].res.splice(index, 1)
          // $http.post('/app/api/v1/ins/update_config', scope.insAccount)
        }
      }
      ...
      scope.addResLevel = (type) => {
        ...
        let level = scope.insAccount.config.message.insUsers[resourceType].level
        if (level < 10) {
          ++scope.insAccount.config.message.insUsers[resourceType].level
          // $http.post('/app/api/v1/ins/update_config', scope.insAccount)
        }
      }
      ...
      scope.save = function() {
        let insUsers = scope.insAccount.config.message.insUsers
